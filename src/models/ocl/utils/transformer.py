#modified from source: https://github.com/singhgautam/slate/blob/master/transformer.py

from models.ocl.utils.utils import *
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None, return_attn = False):
        ...
        if return_attn:
            #sum over all heads
            attn = torch.sum(attn, dim=1)/self.num_heads
            return output, attn
        else:
            return output
...
class ContextEncoderBlock(nn.Module):
    ...
    def __init__(self, d_model, num_heads, dropout=0., gain=1., is_first=False):
        ...
        self.cross_attn = MultiHeadAttention(d_model, num_heads, dropout, gain)
        self.is_first = is_first
        self.ffn_layer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, input, context, is_first=False):
        """
        input: batch_size x target_len x d_model
        context: batch_size x source_len x d_model
        context_mask: batch_size x source_len
        return: batch_size x target_len x d_model
        """
    ...
    def get_attn(self, input, context, attns_list, context_mask=None):
        """
        input: batch_size x target_len x d_model
        context: batch_size x source_len x d_model
        context_mask: batch_size x source_len
        ...
        x = self.encoder_decoder_attn_layer_norm(input)
        x, attn = self.cross_attn(x, context, context, context_mask, return_attn=True)
        input = input + x
        ...
        return input, attns_list
    

class ContextEncoder(nn.Module):
    
    def __init__(self, num_blocks, d_model, num_heads, dropout=0., gain=1.):
        super().__init__()
        
        self.blocks = nn.ModuleList(
            [ContextEncoderBlock(d_model, num_heads, dropout, gain, is_first = True)]+
            [ContextEncoderBlock(d_model, num_heads, dropout, gain, is_first=False) for _ in range(num_blocks - 1)]
            )
    ...
    def forward(self, input, context, context_mask=None):
        ...
        for block in self.blocks:
            input = block(input, context, is_first=False)
        return self.layer_norm(input)

    def get_attn(self, input, context, context_mask=None):
        """
        input: batch_size x target_len x d_model
        context: batch_size x source_len x d_model
        context_mask: batch_size x source_len
        ...
        attns_list = []
        for block in self.blocks:
            input, attns_list = block.get_attn(input, context, attns_list, context_mask)
        return  attns_list
...
def main():
    ...
    context_encoder= ContextEncoder(2, 4, 2)

    op = context_encoder(a, context)
    print(op.shape)
